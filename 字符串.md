...
*ch[i][j]表示Trie中结点i的孩子j的编号
*val[i]>0表示结点i为一个单词的结尾结点
...
* insert(s,n,x) 插入的字符串为s_0,\cdots ,s_{n-1}，长度为n，当前是x位（其中x和u是树上对应的位置）
...
* 长度为len字符串的最短循环节长度为len-Next[len]
*Next[i]表示i-1为结尾的字符串的最长公共前后缀（不算本身）
* 前缀i的最长循环节长度为i-f[i](不算本身)
...
*next[i]表示模式串T[0\cdots (m-1)]和T[i\cdots (m-1)]的最长公共前缀。
*extand[i]表示模式串T[0\cdots (m-1)]和主串S[i\cdots (m-1)]的最长公共前缀。
...
* 以n为结尾的所有回文串按照长度排序以后，可以得到k(k\le log_2n)个按照长度的等差数列 。
